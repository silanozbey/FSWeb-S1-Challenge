...
function indekseGoreFenomen(fenomenler,index) {
    var profile = fenomenler[index].profile;
...
function profilListesi(fenomenler) {
  var kopya = fenomenler.slice();
  var yenisi=[];
 for (var i=0; i<kopya.length;i++){
yenisi.push(kopya[i].profile);
 }
 return yenisi;
}
...
function enFenomenler(fenomenler) {
  ...
  for(var i=0;i<fenomenler.length;i++){
    if (fenomenler[i].followers>100000000){
      milyonluk.push(fenomenler[i].profile);
    }
  }
  return milyonluk;
}
...
function platformaGoreCokGonderiYapanFenomen(fenomenler,platform){
  ...
  for(var i=0;i<fenomenler.length;i++){
    if(fenomenler[i].platform==platform){
      grup.push(fenomenler[i].posts);
    var enYuksek=Math.max(...grup);
    if(fenomenler[i].posts==enYuksek){
      var istenen=fenomenler[i].profile;
    }
    }
  }
  return istenen;
